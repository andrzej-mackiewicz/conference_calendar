...
if($data['post_type'] == 'add'){
  ...
  VALUES ('{$data['user']}', '{$data['judge']}', '{$data['signature']}', '{$data['date']}', '{$data['time_from']}', '{$data['time_to']}', '{$data['room']}', '{$emails_encode}', '{$data['status']}', '{$data['add_date']}', '{$data['confirm']}' )";
...
if($data['post_type'] == 'edit'){
  ...
  if($data['admin'] == 1) {
    $emails_encode = json_encode($data['emails'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    $sql = "UPDATE calendar 
    SET user='{$data['user']}', judge='{$data['judge']}', signature='{$data['signature']}', date='{$data['date']}', time_from='{$data['time_from']}', time_to='{$data['time_to']}', room='{$data['room']}', emails='{$emails_encode}', confirm='{$data['confirm']}'
    WHERE id='{$data['id']}'";
...
if($data['post_type'] == 'accept'){
  ...
  $sql = "UPDATE calendar 
  SET status='1', link='{$data['link']}', last_accept='{$data['last_accept']}'
  WHERE id='{$data['id']}'";
